...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for k in range(3):
            for n in range(3):
                if (i + k < self.width) and (i + k >= 0) and (j + n >= 0) and (j + n < self.height):
                    if not((i+k, j+n) in self.moves_made):
                        neighborcells.add((i+k, j+n))

        self.knowledge.append(
            Sentence(neighborcells, count)
        )
        ...
        for sent0 in base:
            if sent0.count == 0:
                continue
            for sent1 in base:
                if sent0 == sent1 or sent1.count == 0:
                    continue
                if sent0.cells.issubset(sent1.cells):
                    newcells = sent1.cells.difference(sent0.cells)
                    neew = copy.deepcopy(newcells)
                    newcount = sent1.count - sent0.count
                    for move in neew:
                        if move in self.moves_made:
                            newcells.remove(move)
                    if newcells:
